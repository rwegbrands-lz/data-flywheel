...
export default function FlywheelDiagram({ risk, activeSection, onSectionClick }) {
  const cx = 150, cy = 150, innerR = 44, outerR = 96
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <svg width="100%" viewBox="0 0 300 300" style={{ maxWidth: 240, display: 'block', margin: '0 auto', overflow: 'visible' }}>
        {SECTIONS.map(sec => {
          ...
          const labelPos = polarToCartesian(cx, cy, outerR + 28, midAngle)
          return (
        ...
        {scoreIcon ? (
          <>
            <text x={cx} y={cy - 10} textAnchor="middle" style={{ fontSize: 15, fill: scoreColor, fontWeight: 700, fontFamily: 'Figtree, system-ui, sans-serif' }}>{scoreIcon}</text>
            <text x={cx} y={cy + 5} textAnchor="middle" style={{ fontSize: 9, fill: scoreColor, fontWeight: 700, fontFamily: 'Figtree, system-ui, sans-serif' }}>
              {risk.riskScore}
            </text>
            {risk.confidence != null && (
              <text x={cx} y={cy + 17} textAnchor="middle" style={{ fontSize: 7.5, fill: '#94A3B8', fontFamily: 'Figtree, system-ui, sans-serif' }}>
                {Math.round(risk.confidence * 100)}% conf.
              </text>
            )}
          </>
        ) : (
          <text x={cx} y={cy} textAnchor="middle" dominantBaseline="middle" style={{ fontSize: 9, fill: '#64748B', fontWeight: 600, fontFamily: 'Figtree, system-ui, sans-serif' }}>
