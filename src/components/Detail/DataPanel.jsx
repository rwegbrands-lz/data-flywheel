...
function SectionLabel({ children }) {
  return <div style={{ fontSize: 13, fontWeight: 700, color: '#475569', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8, marginTop: 18 }}>{children}</div>
}
    ...
    >
      ...
      {showTooltip && (
        <div style={{
          ...
          padding: '8px 12px', fontSize: 13, minWidth: 220, maxWidth: 280,
    ...
    <div style={{ padding: '0 0 20px 0' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
        {dataPoints.length > 0
          ...
          : <span style={{ fontSize: 14, color: '#475569' }}>No data points defined yet.</span>
        }
      </div>
        ...
        <>
          ...
          {primarySources.map((s, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: SOURCE_COLORS[s.type] || '#475569' }} />
      ...
      {confidence != null && (
        <>
          ...
          <div style={{ fontSize: 13, color: '#475569', marginTop: 4 }}>{Math.round(confidence * 100)}% confidence</div>
        </>
      )}
      ...
      {riskScore && currentLogic && (
        <>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: SCORE_COLORS[riskScore] }}>{currentLogic.label}</div>
            <div style={{ fontSize: 13, color: '#374151', marginTop: 4 }}>{currentLogic.humanReadable}</div>
      ...
      {currentLogic?.pseudoFunction && (
        <>
          <div
            ...
            style={{ cursor: 'pointer', fontSize: 13, color: '#FF6B2C', marginTop: 12, userSelect: 'none' }}
          >
          ...
          {logicOpen && (
            <pre style={{
              ...
              fontSize: 13, overflow: 'auto', marginTop: 6,
        ...
        <>
          ...
          {qualityAssurance.openFlags.map((flag, i) => (
            <div key={i} style={{ background: '#FFFBEB', border: '1px solid #FEF3C7', borderRadius: 8, padding: '8px 12px', marginBottom: 8 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#92400E' }}>{flag.flag}</div>
              <div style={{ fontSize: 13, color: '#78350F', marginTop: 2 }}>{flag.detail}</div>
            </div>
          ))}
        </>
      )}
